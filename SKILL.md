...
payment:
  networks:
    - {stellar | base | stellar-testnet | base-sepolia}
  asset: USDC
...
- [ ] `payment.networks` has at least one valid network
- [ ] `payment.payTo` has a valid wallet address
...
| `MISSING_NETWORKS` | At least one network required |
| `INVALID_NETWORK` | Must be: stellar, base, stellar-testnet, base-sepolia |
| `MISSING_PAY_TO` | Recipient address required |
